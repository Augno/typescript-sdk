...
export async function initMcpServer(params: {
  ...
  let _client: AugnoClient | undefined;
  let _clientError: Error | undefined;
  let _logLevel: 'debug' | 'info' | 'warn' | 'error' | 'off' | undefined;

  const getClient = (): AugnoClient => {
    if (_clientError) throw _clientError;
    if (!_client) {
      try {
        _client = new AugnoClient({
          ...{ environment: (readEnv('AUGNO_CLIENT_ENVIRONMENT') || undefined) as any },
          logger,
