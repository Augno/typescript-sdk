...
import { ClientOptions } from 'augno';

const prompt = `Runs JavaScript code to interact with the Augno Client API.
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    AUGNO_API_KEY: readEnv('AUGNO_API_KEY') ?? client.apiKey ?? undefined,
    AUGNO_CLIENT_BASE_URL:
      readEnv('AUGNO_CLIENT_BASE_URL') ?? readEnv('AUGNO_CLIENT_ENVIRONMENT') ?
        undefined
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    body: JSON.stringify({
      ...
      code,
      intent,
      client_opts: { environment: (readEnv('AUGNO_CLIENT_ENVIRONMENT') || undefined) as any },
