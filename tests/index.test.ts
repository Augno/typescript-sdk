...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new AugnoClient({ logger: logger, logLevel: 'debug', apiKey: 'My API Key' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new AugnoClient({ logger: logger, logLevel: 'info', apiKey: 'My API Key' });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['AUGNO_CLIENT_LOG'] = 'debug';
      const client = new AugnoClient({ logger: logger, logLevel: 'off', apiKey: 'My API Key' });
  ...
  describe('baseUrl', () => {
    ...
    test('env variable with environment', () => {
      ...
      const client = new AugnoClient({ apiKey: 'My API Key', baseURL: null, environment: 'production' });
      expect(client.baseURL).toEqual('https://api.augno.com');
    });
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new AugnoClient({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new AugnoClient({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4 });
